    ...
    function UserAccountLayout({
        ...
        return (
            <div className="w-265 montserrat text-sm text-[#646464] font-semibold">
                <div className="w-277">
                    <div className="overflow-x-auto">
                        ...
                        {users.length === 0 ? (
                            ...
                            <div
                                key={u.id}
                                className={`grid ${editMode ? "grid-cols-[auto_147px_217px_286px_80px_100px_140px_120px_auto]" : "grid-cols-[152px_222px_296px_80px_103px_150px_123px_auto]"} p-3 border-b border-gray-100 items-start mr-4 ${editMode && selectedIds.includes(u.id) ? "bg-[#EAF5FB]" : ""}`}
                            >
                                {editMode && (
                                    <div className="px-2 flex items-center">
                                ...
                                <div className="px-2">
                                    <span className="bg-[#FFF6D4] text-[#FFCC00] px-3 py-1 rounded-lg text-xs">{u.role}</span>
                                </div>
                                <div className="px-2 flex justify-center relative items-start">
                                    <button onClick={() => setActiveMenu(activeMenu === u.id ? null : u.id)} className="text-lg font-bold">•••</button>
                                    {activeMenu === u.id && (
                                        <div data-popup="true" className="absolute -right-7 -top-9 bg-white shadow-md border border-[#646464] rounded-md w-40 z-20">
                                            <button data-popup="true" onClick={() => { setSelectedUser(u); setActiveMenu(null); }} className="text-[#646464] w-full px-3 py-2 hover:bg-gray-100 hover:rounded-md text-left">
                                                View User Detail
                                            </button>
                                        </div>
                                    )}
