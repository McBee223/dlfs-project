...
function ViewItemPopup({ onClose, onSave, item, onRestore }) {
        ...
        <>
            ...
            <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
                ...
                >
                    ...
                    <div className={`overflow-y-auto pr-2 space-y-4 ${categoryOpen ? "pb-15" : ""}`}>
                        <div>
                            <label className="text-sm font-medium mb-1 block">Image</label>
                            {isEditing ? (
                                <label className="flex flex-col items-center justify-center w-full h-44 border-2 border-dashed border-[#047EAF] rounded-xl cursor-pointer bg-[#E8F7FF]">
                        ...
                        <div>
                            <label className="text-sm font-medium mb-1 block">Item Number</label>
                            <input type="text" value={itemNumber} disabled className="input w-full bg-white" />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                disabled={!isEditing}
                                className={`input w-full ${isEditing ? "bg-gray-100" : "bg-white"}`}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <div className="relative" ref={categoryRef}>
                                <button
                                    ...
                                    className={`input w-full flex justify-between items-center ${isEditing ? "bg-gray-100 cursor-pointer" : "bg-white cursor-default"}`}
                                >
                                    <span>{category || "Select category"}</span>
                                    <img src={DropdownIcon} className={`w-4 h-4 transition-transform ${categoryOpen ? "rotate-180" : ""}`} />
                                </button>
                                    ...
                                    <div className="absolute top-12 left-0 w-full bg-white border rounded-lg shadow-lg z-50">
                                        {categories.map((c) => (
                                            <button
                                                ...
                                                className="block w-full px-4 py-2 text-left hover:bg-gray-100 hover:rounded-lg"
                                            >
                                                {c}
                                            </button>
                                        ))}
                        ...
                        <div>
                            <label className="text-sm font-medium mb-1 block">Date Found</label>
                            <input
                                type="datetime-local"
                                value={dateFound}
                        ...
                        <div>
                            <label className="text-sm font-medium mb-1 block">Last Seen</label>
                            <input
                                type="text"
                                value={lastSeen}
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                disabled={!isEditing}
                                rows={4}
                                className={`input w-full resize-none ${isEditing ? "bg-gray-100" : "bg-white"}`}
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-5">
                        {onRestore && (
                            <button
                                onClick={onRestore}
                                className="flex items-center gap-2 bg-[#047EAF] text-white text-sm px-4 py-2 rounded-md hover:bg-[#035f85] transition-colors"
                            >
                                ↩ Restore Item
                            </button>
                        )}
