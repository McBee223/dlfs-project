...
function AdminNotificationItem({ notification, onDelete, onRead }) {
    ...
    // Match by checking if the title contains a known key
    const matchedKey = Object.keys(notifConfig).find((key) =>
        title?.includes(key)
    );
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <h3 className="font-medium text-[13px] text-gray-800">{title}</h3>
                ...
                <div className="overflow-hidden transition-all duration-200 ease-in-out max-h-0 opacity-0 group-hover:max-h-8 group-hover:opacity-100">
                    <button
                        ...
                        className="mt-2 text-[11px] px-3 py-1 rounded text-white transition-colors"
