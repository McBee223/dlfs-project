...
function NotificationItem({ notification, onDelete, onRead }) {
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="overflow-hidden transition-all duration-200 ease-in-out max-h-0 opacity-0 group-hover:max-h-8 group-hover:opacity-100">
                    <button
                        ...
                        className="mt-2 text-[11px] px-3 py-1 rounded bg-[#047EAF] text-white hover:bg-[#035d82] transition-colors"
                    >
                        Delete
                    </button>
                </div>
            ...
            {!read && (
                <div className="w-2 h-2 rounded-full bg-[#378ADD] shrink-0 mt-1.5" />
            )}
        </div>
    );
}
