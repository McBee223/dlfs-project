...
function ItemsCard({ item, category, description, image, onViewDetails, pinnedIds, onPinChange, onClaim, onEditClaim, approvedClaimsCount, hasCancelNotif, onDismissCancelNotif }) {
    ...
    const renderActionButton = () => {
        if (isAlreadyClaimed) return (
            <span className="text-sm font-medium text-gray-400 px-3 py-2">✗ Already Claimed</span>
        );
        ...
        if (isPending || claimStatus === 'In Process') return (
            <button
                ...
                className="text-sm border border-orange-400 text-orange-500 px-3 py-2 rounded-xl hover:bg-orange-50 font-medium"
            >
                Edit Submission
            </button>
        );
        ...
        if (isRejected) return (
            <button
                ...
                className="text-sm border border-red-400 text-red-500 px-3 py-2 rounded-xl hover:bg-red-50 font-medium"
            >
                Re-claim
            </button>
        );
        return (
            <button
                onClick={(e) => { e.stopPropagation(); onClaim?.(item); }}
                className="text-sm border border-[#047EAF] text-[#047EAF] px-3 py-2 rounded-xl hover:bg-yellow-50"
            >
                View Details
            </button>
        );
    ...
    return (
        <div
            ...
            className="montserrat bg-white rounded-2xl p-3 w-full max-w-xs flex flex-col transition-transform duration-200 ease-in-out hover:scale-[1.03] cursor-pointer relative"
        >
            {hasCancelNotif && (
                <div
                ...
                >
                    <div className="w-4 h-4 bg-[#047EAF] rounded-full shadow-md cursor-pointer animate-pulse" />
                    <div className="absolute right-0 top-6 w-60 bg-[#047EAF] text-white text-xs px-3 py-2.5 rounded-xl shadow-lg opacity-0 group-hover/notif:opacity-100 transition-opacity duration-200 pointer-events-none z-30 leading-relaxed">
            ...
            <div className="relative w-full h-36 bg-gray-200 rounded-xl overflow-hidden">
                ...
                {approvedClaimsCount > 0 && !isAlreadyClaimed && (
                    <span className="absolute top-2 right-2 bg-white text-[#047EAF] text-[10px] font-bold px-2 py-0.5 rounded-full shadow border border-[#047EAF]">
            ...
            <div className="flex items-center justify-between mt-auto pt-4">
                ...
                >
                    <img src={isPinned ? PinIconActive : PinIcon} alt="pin" />
                </button>
            </div>
        </div>
    );
