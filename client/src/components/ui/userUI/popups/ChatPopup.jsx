...
function ChatPopup({ onClose }) {
    ...
    return (
        <div className="fixed bottom-6 right-5 w-190 h-140 bg-white rounded-2xl rounded-br-none shadow-2xl flex flex-col overflow-hidden z-50 animate-popup montserrat">
            <div className="p-3 px-5 flex justify-between items-center bg-[#F9F9F9]">
                <span className="font-semibold text-md text-[#047EAF]">
                    Frequently Asked Question Chatbot
                </span>
                <button
                    onClick={onClose}
                    className="text-[#047EAF] font-bold text-md"
                >
                    ✕
                </button>
            </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                <div className="w-[45%] flex flex-col bg-[#fafafa] rounded-md">
                        ...
                        {faqData.map((cat, i) => (
                            <div key={i}>
                                <button
                                    ...
                                    className={`w-full montserrat font-semibold flex items-center justify-between text-left text-sm p-3 rounded-md transition
                                    ${activeCategory === i
                                    ...
                                    <div className="ml-5 max-h-60 overflow-y-auto custom-scrollbar space-y-2.5 pr-1">
                                        {cat.questions.map((q, idx) => (
                                            <button
                                                ...
                                                className="w-full montserrat text-left text-[#047EAF] font-semibold text-[12px] bg-[#D0EDFB] px-2 py-2 rounded-md hover:bg-[#AFE5FF]"
                                            >
                                                {q.q}
                                            </button>
                                        ))}
                                ...
                                return (
                                    <div key={i} className="flex justify-start">
                                        <div className="bg-[#E6EEF3] px-4 py-3 rounded-2xl flex gap-1">
                                            <span className="w-2 h-2 bg-[#1E7FA8] rounded-full animate-bounce"></span>
                                            <span className="w-2 h-2 bg-[#1E7FA8] rounded-full animate-bounce [animation-delay:0.2s]"></span>
                                            <span className="w-2 h-2 bg-[#1E7FA8] rounded-full animate-bounce [animation-delay:0.4s]"></span>
                                        </div>
                                    </div>
                                );
                            }
